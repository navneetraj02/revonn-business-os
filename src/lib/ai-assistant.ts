...
export async function generateAIResponse(
  ...
  try {
    switch (intent) {
      ...
      case 'sales_today':
        return await getSalesTodayResponse(lang);
      
      case 'profit_today':
        return await getProfitResponse(lang);
      
      case 'stock_check':
        return await getStockCheckResponse(message, lang);
      
      case 'low_stock':
        return await getLowStockResponse(lang);
      
      case 'reorder':
        return await getReorderResponse(lang);
      
      case 'item_sales':
        return await getItemSalesResponse(message, lang);
      
      case 'create_bill':
        return getBillCreationResponse(message, lang);
      
      case 'customer_history':
        return await getCustomerHistoryResponse(message, lang);
      
      case 'customer_credit':
        return await getCustomerCreditResponse(message, lang);
      
      case 'marketing_message':
        return getMarketingResponse(message, lang);
      
      case 'gst_help':
        return getGSTHelpResponse(message, lang);
      
      case 'staff_attendance':
        return await getStaffAttendanceResponse(lang);
      
      case 'staff_salary':
        return await getStaffSalaryResponse(message, lang);
      
      case 'bom_help':
        return getBOMHelpResponse(lang);
      
      case 'app_help':
        return getAppHelpResponse(message, lang);
      
      default:
        return getUnknownResponse(lang);
    }
  } catch (error) {
